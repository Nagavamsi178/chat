...
def get_first_user_message(messages_json):
    """Extract the first user message to use as chat title"""
    try:
        messages = json.loads(messages_json)
        for msg in messages:
            if msg.get("role") == "user":
                # Return first 30 characters of the first user message
                user_message = msg.get("content", "")[:30]
                return user_message + "..." if len(user_message) == 30 else user_message
    except:
        pass
    return "New Chat"
...
def main():
    ...
    if "current_chat_title" not in st.session_state:
        st.session_state.current_chat_title = generate_session_title()
    ...
    with st.sidebar:
        ...
        if chats:
            ...
            for chat_id, title, messages, timestamp in chats:
                ...
                display_title = get_first_user_message(messages)
